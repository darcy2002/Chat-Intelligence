...
export const grokProvider: LLMProvider = {
  ...
  async chat(message: string, history: Message[]) {
    try {
      // Preferred path: structured outputs via OpenAI-compatible response_format
      try {
        ...
        throw new Error("Grok returned no parsed message");
      } catch (structuredErr) {
        console.warn(
          `[grok] structured outputs failed (${
      ...
      console.error(
        `[grok] error, falling back to logic: ${err instanceof Error ? err.message : String(err)}`
      );
